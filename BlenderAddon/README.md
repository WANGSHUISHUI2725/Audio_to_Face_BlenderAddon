...
2. Select `D:\A2F_Dev\BlenderAddon\a2f_local.zip` and enable **Audio2Face Local**.
3. Verify the model, CUDA, and TensorRT paths in the add-on preferences. The exporter and
   `audio2x.dll` are included in the ZIP.
...
files are stored persistently in `D:\A2F_Dev\BlenderAddon\generated_json`; the folder can
be changed in the add-on preferences and opened from the Audio2Face panel.
